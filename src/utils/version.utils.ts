 ...
 *
 * @see https://github.com/yarnpkg/berry/blob/master/packages/plugin-version/sources/versionUtils.ts
 */
...
export type Releases = Map<Workspace, Exclude<Decision, Decision.UNDECIDED>>;
...
export async function fetchRoot(initialCwd: PortablePath) {
  ...
  do {
    cwd = nextCwd;
    if (await xfs.existsPromise(ppath.join(cwd, `.git` as Filename))) match = cwd;
...
export async function resolveVersionFiles(project: Project) {
  ...
  const deferredVersionFolder: any = project.configuration.get(`deferredVersionFolder`);
  if (!xfs.existsSync(deferredVersionFolder)) return new Map();
  ...
  for (const entry of deferredVersionFiles) {
    ...
    for (const [identStr, decision] of Object.entries(versionData.releases || {})) {
      ...
      if (workspace === null)
        throw new Error(
          `Assertion failed: Expected a release definition file to only reference existing workspaces (${ppath.basename(
            versionPath,
          )} references ${identStr})`,
        );
      ...
      if (workspace.manifest.version === null)
        throw new Error(
          `Assertion failed: Expected the workspace to have a version (${structUtils.prettyLocator(
      ...
      if (suggestedRelease === null)
        throw new Error(
          `Assertion failed: Expected ${workspace.manifest.version} to support being bumped via strategy ${decision}`,
        );

      const bestRelease =
        typeof candidateRelease !== `undefined`
          ? semver.gt(suggestedRelease, candidateRelease)
            ? suggestedRelease
            : candidateRelease
          : suggestedRelease;
...
export async function updateVersionFiles(project: Project) {
  ...
  for (const entry of deferredVersionFiles) {
    ...
    await xfs.changeFilePromise(versionPath, stringifySyml(new stringifySyml.PreserveOrdering(versionData)));
  }
}
...
export async function openVersionFile(project: Project, { allowEmpty = false }: { allowEmpty?: boolean } = {}) {
  ...
  const base = root !== null ? await fetchBase(root, { baseRefs: configuration.get(`changesetBaseRefs`) }) : null;

  const changedFiles = root !== null ? await fetchChangedFiles(root, { base: base!.hash, project }) : [];

  const deferredVersionFolder: any = configuration.get(`deferredVersionFolder`);
  ...
  const versionPath =
    versionFiles.length === 1
      ...
      : ppath.join(
          ...
          `${hashUtils.makeHash(Math.random().toString()).slice(0, 8)}.yml` as Filename,
        );

  const versionContent = xfs.existsSync(versionPath) ? await xfs.readFilePromise(versionPath, `utf8`) : `{}`;
...
export function getUndecidedDependentWorkspaces(
  versionFile: Pick<VersionFile, 'project' | 'releases'>,
  { include = new Set() }: { include?: Set<Workspace> } = {},
) {
  const undecided = [];

  const bumpedWorkspaces = new Map(
    miscUtils.mapAndFilter([...versionFile.releases], ([workspace, decision]) => {
      if (decision === Decision.DECLINE) return miscUtils.mapAndFilter.skip;

      return [workspace.anchoredLocator.locatorHash, workspace];
    }),
  );

  const declinedWorkspaces = new Map(
    miscUtils.mapAndFilter([...versionFile.releases], ([workspace, decision]) => {
      if (decision !== Decision.DECLINE) return miscUtils.mapAndFilter.skip;
...
export function suggestStrategy(from: string, to: string) {
  ...
  for (const strategy of Object.values(Decision))
    if (strategy !== Decision.UNDECIDED && strategy !== Decision.DECLINE)
      if (semver.inc(from, strategy) === cleaned) return strategy;

  return null;
}

export function applyStrategy(version: string | null, strategy: string) {
  if (semver.valid(strategy)) return strategy;

  if (version === null)
    throw new UsageError(
      `Cannot apply the release strategy "${strategy}" unless the workspace already has a valid version`,
    );
...
export function applyReleases(project: Project, newVersions: Map<Workspace, string>, { report }: { report: Report }) {
  const allDependents: Map<Workspace, Array<[Workspace, AllDependencies, IdentHash]>> = new Map();
  ...
  for (const [workspace, newVersion] of newVersions) {
    ...
    const identString = workspace.manifest.name !== null ? structUtils.stringifyIdent(workspace.manifest.name) : null;
    ...
    for (const [dependent, set, identHash] of dependents) {
      ...
      if (!parsed) {
        report.reportWarning(
          MessageName.UNNAMED,
          `Couldn't auto-upgrade range ${range} (in ${structUtils.prettyLocator(
            project.configuration,
            workspace.anchoredLocator,
          )})`,
        );
